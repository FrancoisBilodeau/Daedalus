...
public enum CursorState{draw,start,end,none};
public enum ErrorType{Start,End,Inc,NameTaken,NameWrong,NameEmpty,None};
...
public class TileMapMouse : MonoBehaviour {
	...
	void Update () {
		...
		if(Input.GetMouseButton(1)&& GUIUtility.hotControl == 0) {
			if( GetComponent<Collider>().Raycast( ray, out hitInfo, Mathf.Infinity ) ) {
				...
				if(((x > -1  && z > -1) && (x < (maze.sizeX) && z < (maze.sizeZ))) && state == CursorState.draw && maze.mapData[z,x] != true)
					AddTile(x ,z ,maze );				
		...
		if(Input.GetMouseButton(0)&& GUIUtility.hotControl == 0) {
			if( GetComponent<Collider>().Raycast( ray, out hitInfo, Mathf.Infinity ) ) {
				...
				if(((x > -1  && z > -1) && (x < (maze.sizeX) && z < (maze.sizeZ))) && state == CursorState.draw && maze.mapData[z,x] != false)
					RemoveTile(x ,z ,maze );
				//Vérif si c'est pas un mur de CONTOUR et du state
				if(((x > -1  && z > -1) && (x < (maze.sizeX) && z < (maze.sizeZ))) && state == CursorState.start)
					PutStart(x ,z ,maze);
	...
	private void InvertGrille(Maze maze){
		...
		for(int i = 0; i < 64; i++){
			for(int k = 0; k < 64; k++){
				if(maze.mapData[k,i] == false){
					maze.mapData[k,i] = true;					
				}else if(maze.mapData[k,i] == true){
					maze.mapData[k,i] = false;	
				}
			}
		}
	...
	private void DoMyWindow (int windowID) {
		...
		if (GUI.Button (new Rect (90,90,150,20), "Save")){
			...
			}else if(verif.verification(maze) == Verification.VerificationResult.StartInvalide){
				err = ErrorType.Start;
			}else if(verif.verification(maze) == Verification.VerificationResult.EndInvalide){
				err = ErrorType.End;
			}else{
				err = ErrorType.Inc;
			}
